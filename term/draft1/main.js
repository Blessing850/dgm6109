...
if (hour < 23 && hour >= 6) {
    category = "before11"        // normal night sleep (6pm–10pm)
} 
else if (hour >= 23 || hour === 0 || hour === 1) {
    category = "11to1"           // 11pm–1am
} 
else {
    category = "after1"          // 2am+
}
...
viz.append("g")
    ...
    // JITTER FUNCTION (reduces overlap)
    function jitter() {
        return (Math.random() - 0.5) * 12
    }
    // SORT data by size so small circles are drawn on top
    data.sort((a, b) => b.minutesAwake - a.minutesAwake)
    ...
    viz.selectAll("circle")
        ...
        .enter()
        .append("circle")
        .attr("cx", d => xScale(d.hours) + jitter())
        .attr("cy", d => yScale(d.quality) + jitter())
        .attr("r", d => rScale(d.minutesAwake))
        .attr("fill", d => colorScale(d.bedtimeCategory))
        .attr("stroke", "black")
        .attr("stroke-width", 1)
        .attr("opacity", 0.75)
...
function drawLegend() {
    ...
    let sizeCircles = sizeLegend.selectAll("circle")
    .data(sizes)
    .enter()
    .append("circle")
    .attr("cy", (d, i) => i * 70 + rScale(d)) // space circles vertically
    .attr("r", d => rScale(d))
    .attr("fill", "gray")
    .attr("opacity", 0.6);
...
sizeLegend.selectAll("text.sizeLabel")
    ...
    .attr("x", 45)
    .attr("y", (d, i) => i * 70 + rScale(d) + 5) // just below each circle
